...
ip_iface_alloc(const char *unicast, const char *netmask)
{
    ...
    /* Exercise 7-3 */
    ip_addr_t unicast_bin, netmask_bin;
    if (ip_addr_pton(unicast, &unicast_bin) == -1) {
        errorf("unicast(%s) can not convert to binary", unicast);
        memory_free(iface);
        return NULL;
    }
    iface->unicast = unicast_bin;
    if (ip_addr_pton(netmask, &netmask_bin) == -1) {
        errorf("netmask(%s) can not convert to binary", netmask);
        memory_free(iface);
        return NULL;
    }
    iface->netmask = netmask_bin;
    iface->broadcast = (unicast_bin & netmask_bin) | (~netmask_bin);
    /* Exercise 7-3 */
    return iface;
}
...
ip_protocol_register(uint8_t type, void (*handler)(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface))
{
    ...
    for (entry=protocols; entry; entry = entry->next) {
        if (entry->type == type) {
            errorf("This type(%u) is already exists", type);
            return -1;
        }
    }
...
ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    if (v != IP_VERSION_IPV4) {
        errorf("IP_VERSION(%u) is not IPV4", v);
        return;
    }
    hlen = (hdr->vhl & 0x0f) << 2;
    if (len < hlen) {
        errorf("len(%zuByte) is less than hedder length(%uByte)", len, hlen);
        return;
    }
    total = ntoh16(hdr->total); // バイトオーダー変換
    if (len < total) {
        errorf("len(%zuByte) is less than total length(%uByte)", len, total);
        return;
    }
    if (cksum16((uint16_t *)data, len, 0)) {
        errorf("check sum failure");
        return;
    }
    offset = ntoh16(hdr->offset);
    ...
    if (iface == NULL){
        errorf("net_device has not ip interface");
        return;
    }
...
ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    if (src == IP_ADDR_ANY) {
        ...
        if (!iface) {
            errorf("no interface found at %s", src);
            return -1;
        }
        if (dst != IP_ADDR_BROADCAST && (dst & iface->netmask) != (iface->unicast & iface->netmask)) {
            errorf("this addr(%s) is no route to host", ip_addr_ntop(dst, addr, sizeof(addr)));
            return -1;
        }
    }
